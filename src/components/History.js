...
export default function History() {
    ...
    function disableDay({ date, view }) {
        if (view === "month") {
            if (
                history.find((day) => day.day === dayjs(date).format("DD/MM/YYYY")) ||
    ...
    function renderClickedDayHabits(value) {
        ...
        setClickedDay(
            <ClickedDayHabitsContainer>
                ...
                <p>
                    {Number(
                        (clickdDate.habits.filter((habit) => habit.done).length * 100) / clickdDate.habits.length
                    ).toFixed(0)}
                    % de hábitos concluidos
                </p>
                {clickdDate.habits.map((habit, i) => (
                    <HabitClickedDay key={i} name={habit.name} wasDone={habit.done} />
                ))}
            </ClickedDayHabitsContainer>
        );
    }
